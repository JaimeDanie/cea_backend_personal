...
export class AuthService {
  ...
  async changePassword({ password, confirmation, code }: ChangePasswordDto) {
    if (password !== confirmation) {
      throw new BadRequestException();
    }
    const user = await this.userService.getUserByRecovery(code);
  ...
  async requestChangePassword(email: string): Promise<User> {
    const user = await AppDataSource.manager.findOne(User, {
      where: { email },
    });
    if (!user) {
    ...
    user.recoveryHashPassword = uuidv4();
    this.mailerService.sendMail({
      to: email,
      from: 'Admin muto<jaimedaniel.bm91@gmail.com>',
      subject: 'Reset password',
      html: this.renderHTMLrecoveryPassword(
        'CEA Admin',
        `${user.firstName} ${user.lastName}`,
      ),
    });
    const userSaved = await AppDataSource.manager.save(user);
    delete userSaved['currentToken'];
  ...
  renderHTMLrecoveryPassword(productName: string, name: string): string {
